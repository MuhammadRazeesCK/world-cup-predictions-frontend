...
export function PendingReminderExport({ groups }: { groups: PendingGroup[] }) {
  ...
  const handleWhatsApp = async () => {
    ...
    try {
      await document.fonts.ready;
      const dataUrl = await toPng(cardRef.current, { pixelRatio: 2, backgroundColor: D.bg });
      const res = await fetch(dataUrl);
      const blob = await res.blob();
      const file = new File([blob], 'wc2026-pending.png', { type: 'image/png' });

      // Mobile: native share sheet (sends actual image to WhatsApp)
      const isTouchDevice = window.matchMedia('(hover: none) and (pointer: coarse)').matches;
      if (isTouchDevice && typeof navigator.share === 'function' && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: 'WC 2026 Pending Submissions' });
        return;
      }

      // Desktop: copy image to clipboard only — user pastes into their existing WhatsApp tab
      try {
      ...
      if (e?.name !== 'AbortError') console.error('WhatsApp share error:', e);
    } finally {
      setSharing(false);
    }
  };
